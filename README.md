...
- small watershed note: watersheds with polygon area at or below 10 km2 are marked `tiny_watershed`; the GEE code reduces each image over the watershed polygon, and if that still comes back blank, it retries the same polygon at a finer scale and marks that row with `used_fine_scale_fallback`
