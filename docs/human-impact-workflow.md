...
- Open the notebook and run the setup cells.
- In **Edit Run Settings**, choose sites and change the year lists if needed.
- Set `MAX_TASKS_TO_LAUNCH = 5` for a small test run. This runs one export for
  each dataset. Leave it as `None` for all 38 exports.
- Run the source-data check, launch exports, and use the final cell to watch
  their status.
...
- `colab_notebooks/run_human_impacts.ipynb`: Colab notebook;
