...
era5_run_label <- Sys.getenv("SILICA_ERA5_RUN_LABEL", unset = "AND_fine_scale")
lter_filter <- Sys.getenv("SILICA_ERA5_LTER_FILTER", unset = "AND")
...
output_slug <- Sys.getenv(
  ...
  unset = if (identical(era5_run_label, "AND_fine_scale")) "and" else slugify(era5_run_label)
)
...
today_era5_folder <- file.path(
  ...
  paste0("era5_land_", era5_run_label, "_2001_2023")
)
