...
default_drive_subfolder <- function(slug) {
  case_when(
    identical(slug, "and") ~ "Andrews sites only testing",
    identical(slug, "comparison_sites") ~ "Comparison sites annual ERA5-Land testing",
    identical(slug, "watershed_size_comparison") ~ "qa_outputs_old_vs_gee_era5_land_comparisons",
    identical(slug, "all_sites") ~ "All sites annual ERA5-Land comparison",
    TRUE ~ paste0(slug, " ERA5-Land comparison")
  )
}
...
download_from_drive <- parse_bool_arg("--download-from-drive", TRUE)
...
upload_to_drive <- parse_bool_arg("--upload-to-drive", TRUE)
...
drive_folder_id <- get_arg("--drive-folder-id", "1hYedMgoR1907nwtOjjjqYFzjG28gk3-T")
