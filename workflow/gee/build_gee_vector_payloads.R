...
analysis_scale_text <- cli_value(args, "--analysis-scale-m", "")
analysis_scale_m <- if (nzchar(analysis_scale_text)) {
  as.numeric(analysis_scale_text)
} else {
  NA_real_
}
time_slices_per_task <- cli_integer(
  args,
  "--time-slices-per-task",
  1L,
  minimum = 1L
)
bands_per_slice <- cli_integer(args, "--bands-per-slice", 1L, minimum = 1L)
maximum_effective_pixels <- as.numeric(
  cli_value(args, "--maximum-effective-pixels-per-task", "100000000")
)
...
if (
  (!is.na(analysis_scale_m) && (!is.finite(analysis_scale_m) || analysis_scale_m <= 0)) ||
    !is.finite(maximum_effective_pixels) || maximum_effective_pixels <= 0
) {
  stop("Workload limits and analysis scale must be positive numbers.", call. = FALSE)
}
...
feature_work <- if (is.na(analysis_scale_m)) {
  rep(NA_real_, nrow(payloads))
} else {
  exact_area_km2 * 1e6 / analysis_scale_m^2 *
    time_slices_per_task * bands_per_slice
}
if (
  !is.na(analysis_scale_m) &&
    max(feature_work) > maximum_effective_pixels
) {
  worst <- which.max(feature_work)
  stop(
    "A single watershed (`", payloads$site_id[[worst]], "`) requires about ",
    round(feature_work[[worst]]),
    " pixel-band-time evaluations, above the limit of ",
    format(maximum_effective_pixels, scientific = FALSE),
    ". Increase the analysis scale or shorten the task time window.",
    call. = FALSE
  )
}
...
if (!is.na(analysis_scale_m)) {
  initial_groups <- max(
    initial_groups,
    ceiling(sum(feature_work) / maximum_effective_pixels)
  )
}
...
assign_groups <- function(group_count) {
  ...
  for (index in order_index) {
    ...
    if (!is.na(analysis_scale_m)) {
      target_work <- sum(feature_work) / group_count
      scores <- scores +
        (group_work + feature_work[[index]]) / target_work
    }
    ...
    if (!is.na(analysis_scale_m)) {
      group_work[[selected]] <- group_work[[selected]] + feature_work[[index]]
    }
  }
  assignment
}
...
write_payload_set <- function(group_count) {
  ...
  for (group in seq_len(group_count)) {
    ...
    records[[group]] <- tibble(
      ...
      estimated_pixel_band_time = if (is.na(analysis_scale_m)) {
        NA_real_
      } else {
        sum(feature_work[assignment == group])
      },
...
repeat {
  ...
  size_safe <- max(manifest$bytes) <= maximum_payload_bytes
  work_safe <- is.na(analysis_scale_m) ||
    max(manifest$estimated_pixel_band_time) <= maximum_effective_pixels
  if (size_safe && work_safe) break
...
inventory <- st_drop_geometry(payloads) |>
  transmute(
    ...
    estimated_pixel_band_time = feature_work
  ) |>
  arrange(site_id)
...
write_csv(
  tibble(
    metric = c(
      ...
      "bands_per_slice",
      "maximum_payload_pixel_band_time"
    ),
    value = c(
      nrow(inventory),
      nrow(manifest),
      ...
      if (is.na(analysis_scale_m)) {
        ...
        max(manifest$estimated_pixel_band_time)
      }
    )
  ),
...
if (!is.na(analysis_scale_m)) {
  message(
    "Largest payload pixel-band-time estimate: ",
    round(max(manifest$estimated_pixel_band_time))
  )
}
