...
ABSOLUTE_MAX_BATCH_HOURS = 100.0
ABSOLUTE_MAX_SINGLE_TASK_HOURS = 25.0
UNKNOWN_WORKFLOW_CANCEL_HOURS = 5.0
MAX_ACTIVE_TASKS = 5
...
def evaluate_preflight(
    *,
    ...
    bands_per_slice: int = 1,
    effective_pixels_per_task: float | None = None,
) -> dict[str, Any]:
    blockers: list[str] = []
    warnings: list[str] = []
    ...
    estimate_source = "launcher_override" if effective_pixels is not None else None
    if effective_pixels is None and max_task_area_km2 is not None and scale_m is not None:
        effective_pixels = (
            max_task_area_km2
            * 1_000_000
            / (scale_m * scale_m)
    ...
    if proposed_task_count > MAX_TASKS_PER_LAUNCH:
        blockers.append(
            f"The launch proposes {proposed_task_count} tasks; the hard limit is "
            f"{MAX_TASKS_PER_LAUNCH} tasks per receipt."
        )

    if active_count + proposed_task_count > MAX_ACTIVE_TASKS:
        blockers.append(
            f"{active_count} Earth Engine tasks are already active and "
            f"{proposed_task_count} more are proposed; the hard combined limit "
            f"is {MAX_ACTIVE_TASKS}."
        )
    ...
    if (
        ...
        and observed_hours + projected_hours > stop_hours
    ):
        blockers.append(
            f"Month-to-date usage plus the historical p90 projection is "
            f"{observed_hours + projected_hours:.1f} EECU-h, above the "
            f"{stop_hours:.1f} EECU-h monthly safety stop."
        )
    ...
    if effective_pixels is not None and effective_pixels > MAX_EFFECTIVE_PIXELS_PER_TASK:
        blockers.append(
            ...
            f"pixel-band-time evaluations, above the "
            f"{MAX_EFFECTIVE_PIXELS_PER_TASK:,} safety limit. Split the "
            "sites/geometry, shorten the time slice, remove bands, or coarsen "
            "the scale."
        )
    ...
    if p90_hours is None:
        ...
        watchdog_cancel_hours = min(
            max_single_task_hours,
            max(UNKNOWN_WORKFLOW_CANCEL_HOURS, float(p90_hours) * 3),
        )
...
def validate_preflight_receipt(
    ...
    *,
    ...
    if receipt.get("schema_version") != 2:
        raise RuntimeError(
            "GEE preflight receipt uses an obsolete safety schema. Run a fresh "
            "preflight with the current code."
        )
...
def consume_preflight_receipt(
    ...
    *,
    ...
    if start_watchdog:
        ...
        watchdog_cancel_hours = float(
            receipt.get("decision", {})
            .get("limits", {})
            .get("watchdog_cancel_eecu_hours", UNKNOWN_WORKFLOW_CANCEL_HOURS)
        )
        watchdog_warning_hours = min(10.0, watchdog_cancel_hours / 2)
        watchdog_command = [
            sys.executable,
            str(watchdog_script),
            "--project",
            ...
            description_prefix,
            "--warning-eecu-hours",
            str(watchdog_warning_hours),
            "--cancel-eecu-hours",
            str(watchdog_cancel_hours),
            "--log",
            str(watchdog_log),
        ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bands-per-slice", type=int, default=1)
    parser.add_argument(
        "--effective-pixels-per-task",
        type=float,
        help=(
            "Launcher-computed bounded work estimate. Use for algorithms such "
            "as fixed-size sampling where polygon area divided by scale would "
            "overstate the pixels actually evaluated."
        ),
    )
    parser.add_argument(
        "--workload-fingerprint",
        help="Exact launcher-generated SHA-256 fingerprint for the proposed tasks.",
    )
    parser.add_argument(
        "--monthly-limit-hours",
        type=float,
    ...
    if args.bands_per_slice < 1:
        parser.error("--bands-per-slice must be positive.")
    if (
        args.effective_pixels_per_task is not None
        and args.effective_pixels_per_task <= 0
    ):
        parser.error("--effective-pixels-per-task must be positive.")
    if (
        args.workload_fingerprint is not None
        and not re.fullmatch(r"[0-9a-f]{64}", args.workload_fingerprint)
    ):
        parser.error("--workload-fingerprint must be a lowercase SHA-256 value.")
...
def main() -> int:
    ...
    receipt = build_receipt(
        ...
        bands_per_slice=args.bands_per_slice,
        effective_pixels_per_task=args.effective_pixels_per_task,
