...
read_shared_watershed_aliases <- function(path) {
  ...
  required <- c(
    "LTER", "Shapefile_Name", "source_Stream_Name",
    "alias_Stream_Name", "alias_site_id"
  )
...
add_shared_watershed_aliases <- function(data, alias_file) {
  ...
  for (index in seq_len(nrow(aliases))) {
    ...
    if (any(existing)) {
      stop(
        "GLC data already exists for ",
        alias$LTER,
        "__",
        alias$alias_Stream_Name
      )
    }
