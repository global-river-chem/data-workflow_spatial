"""Finish missing MODIS summary assets in quota-checked batches."""
...
MISSING_PATTERN = re.compile(
    r"Missing tasks: (?P<missing>\d+); eligible in this launch: (?P<batch>\d+)\."
)
...
def wait_for_tasks(task_ids: list[str], poll_seconds: int) -> None:
    ...
    while True:
        ...
        if all(state in terminal for state in state_by_id.values()):
            ...
            if failed:
                raise RuntimeError(
                    "A MODIS task did not complete: " + json.dumps(failed)
                )
            return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--payload-list", type=Path, action="append", required=True)
    ...
    parser.add_argument("--maximum-payload-mb", type=float, default=5.0)
...
def main() -> int:
    args = parse_args()
    launcher = Path(__file__).with_name("run_safe_modis_parity_exports.py")
    receipt_dir = args.receipt_dir or Path(tempfile.mkdtemp(prefix="modis-finish-"))
    receipt_dir.mkdir(parents=True, exist_ok=True)
    ee.Initialize(project=args.project)

    base_command = [sys.executable, str(launcher)]
    for payload_list in args.payload_list:
        base_command.extend(["--payload-list", str(payload_list)])
    base_command.extend(
        [
            "--output-folder",
            args.output_folder,
            ...
            "--maximum-payload-mb",
            str(args.maximum_payload_mb),
            "--max-new-tasks",
            str(args.batch_size),
            "--monthly-stop-eecu-hours",
            str(args.monthly_stop_eecu_hours),
            "--watchdog-cancel-eecu-hours",
            str(args.watchdog_cancel_eecu_hours),
            "--check-existing",
        ]
    )
    for expected_site_ids in args.expected_site_ids or []:
        base_command.extend(["--expected-site-ids", str(expected_site_ids)])
    ...
    while args.max_batches == 0 or completed_batches < args.max_batches:
        ...
        if "No missing tasks remain; nothing was submitted." in plan_output:
            print("All missing MODIS assets are complete.", flush=True)
            return 0
